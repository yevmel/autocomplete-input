(() => {
  ...
  function applySuggestion(autocompleteContainerEl, suggestionEl) {
    autocompleteContainerEl.querySelector(".autocomplete-value").value = suggestionEl.dataset.autocompleteSuggestion;
    const autocompleteTextEl = autocompleteContainerEl.querySelector(".autocomplete-text");
    autocompleteTextEl.value = autocompleteTextEl.dataset.autocompleteCurrentText = suggestionEl.dataset.autocompleteSuggestionText;
  }
  ...
  function setupSuggestionsHtmlFromAddresses(addresses) {
    const rows = addresses.length === 0 ? `<tr><td colspan="999">no suggestions for you \xAF\\_(\u30C4)_/\xAF</td></tr>` : addresses.map((address) => `
  ...
  (function setupAutocomplete(root = document) {
    ...
    root.addEventListener("input", debounce((event) => {
      ...
      if (suggestionsBaseUrl === void 0) {
        return;
      }
      const filter = event.target.value;
      const suggestionsContentType = event.target.dataset.suggestionsContentType || "text/html";
      const suggestionsUrl = suggestionsBaseUrl.includes("?") ? `${suggestionsBaseUrl}&filter=${filter}` : `${suggestionsBaseUrl}?filter=${filter}`;
      if (abortController) {
